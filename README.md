...
## How it works
```
Image → Landmark Detection → Geometric Feature Extraction → Trait Interpretation → Rule-Based Scoring → Ranked Recommendations
```
...
10 normalized ratios computed from landmark coordinates:
...
Each trait triggers weighted scoring adjustments across 8 hairstyle dimensions (`volume_top`, `volume_sides`, `short_sides`, `longer_hair`, `fringe`, `clean_lines`, `soft_texture`, `textured_top`). Rules encode established hairstyling principles — e.g. wide jaw → penalize short sides, boost soft texture.

### 5. Hairstyle Matching
...
│   ├── feedback.py          # session logging and vote tracking
...
│   └── norms/
│       ├── male_norms_p123.csv
│       └── female_norms_p123.csv
│
├── util/
│   ├── compute_norms.py     # dataset norm computation
...
| Norm calibration | pandas (UTKFace dataset) |
| UI | Streamlit |
...
Every session is logged to `data/feedback.csv` with extracted features, quality score, and top recommendations. Users can vote on individual recommendations (thumbs up/down), stored separately in `data/votes.csv`. This enables future weight optimization based on real preference data rather than manual tuning.
...
- **Hair segmentation** — BiSeNet or MediaPipe Hair for accurate forehead detection, hairline analysis, and hair type classification
- **Profile view support** — InsightFace 3D landmarks for jaw angle, chin projection, and lateral nose profile
